...
RUN_MAX = int(os.environ.get("MAX_RUN_AGE_DAYS", "35"))
...
if run_age is None or run_age > RUN_MAX:
    state, why = "stale", (
        "the audit job has not run" if run_age is None
        else f"the audit job last ran {run_age} days ago")
elif data_age > DATA_MAX:
    state, why = "stale", (
        f"the job is running (last run {run_age}d ago) but no new run has been "
        f"published in {data_age} days — check whether the audit is still producing new runs")
else:
    state, why = "fresh", f"job ran {run_age}d ago, data {data_age}d old"

print(f"generated={data['generatedUtc']}")
print(f"week={data['asof']}")
